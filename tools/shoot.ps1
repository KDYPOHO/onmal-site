...
.DESCRIPTION
    ...
    🔴 <b>index.html 은 바닥 잡음이 있습니다.</b> 아무것도 안 바꾸고 두 번 찍어도
    그만큼 다릅니다 — 히어로의 데모 자막이 JS 타이머로 돌고 '듣는 중' 파형이
    애니메이션이라 잡히는 순간이 매번 조금씩 다릅니다. 보통 <b>70~90픽셀</b>이고
    글자 가장자리에만 나타납니다.
    🔴 <b>그 값을 외우지 마십시오 — 그날 재십시오.</b> 하루 안에 84 → 5,757 → 78 로
    움직였습니다(2026-08-03). 5,757 이던 동안에는 사양 h2·로드맵 h2 <b>두 덩어리</b>가
    3,205·2,528픽셀씩, 그것도 <b>매번 같은 개수</b>로 나왔습니다. 스크롤 구동
    (section > h2 의 scene-in)이 <b>전체 페이지 창에서는</b> 스크롤이 없어 진행값이
    붙박이가 되고, transform 이 걸린 채 굳느냐 아니냐로 제목 한 줄의 래스터가 통째로
    갈리기 때문입니다. 제목 크기를 올리자 걸리는 자리가 옮겨가며 다시 78 로 내려왔습니다.
    ⚠️ 그래서 <b>"덩어리로 뭉쳐 있으면 진짜 변화"는 믿을 수 없습니다.</b>
       판정법은 하나뿐입니다 — <b>같은 원본을 두 번 찍어</b> 견주고
       (<c>-Label X2 -CompareWith X</c>) 그날의 바닥을 먼저 잽니다.
...
$pages = @(
    ...
    # (#shots · #how 각각 상자 min(100vh,1000px) + 늘린 자리 min(180vh,1500px)
    # · min(150vh,1200px)) 까지 더해 <b>15,481px</b> 입니다 (2026-08-03 실측 —
    # 창을 1,100 으로 잡든 1,500 으로 잡든 같은 값이라 상한 넷이 다 걸린 것을
    # 확인했습니다).
    ...
    # 🔴 <b>구간을 하나 더 붙이거나 제목 크기를 올릴 때마다 여기를 올리십시오.</b>
    #    #how 를 붙이면서 1,176px, 상자를 화면 높이로 세우고 제목을 애플 층위로
    #    올리면서 865px 이 더 자랐습니다. 잊으면 아래 세 섹션이 조용히 잘립니다.
    @{ File = 'index.html';       Height = 16000; Route = '' },
